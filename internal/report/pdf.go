...
// embedImage registers and draws a PNG image. The image type is inferred from
// the file extension. Errors (missing file, unsupported format) are silently
// ignored so a missing chart doesn't abort the whole PDF.
func embedImage(pdf *gofpdf.Fpdf, path string, x, y, w, h float64) {
	...
	// Re-register using gofpdf's image reader (needs the file again).
